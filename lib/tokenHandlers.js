const { createRandomString, toHash } = require('./utils.js')
...
tokenHandlers.verifyToken = async (token, phone) => {
  try {
...
tokenHandlers.get = async ({ queryParams }) => {
...
tokenHandlers.put = async ({ body }) => {
  const { shouldExtend, token } = body;
...
tokenHandlers.delete = async ({ queryParams }) => {
  const { token } = queryParams;
  try {
